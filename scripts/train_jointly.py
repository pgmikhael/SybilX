...
import sybilx.utils.losses as losses
import sybilx.utils.metrics as metrics
import sybilx.utils.loading as loaders
import sybilx.models as models

from sybilx.parsing import parse_args
from scripts.train import SybilLightning, prefix_dict
...
class SybilLightningAdapt(SybilLightning):
    ...
    def step(self, batch, batch_idx, optimizer_idx, log_key_prefix=""):
        ...
        if optimizer_idx == 0 or optimizer_idx is None:
            ...
            if self.args.save_attention_scores:
                attentions = {
                    ...
                    if any([attn_key in k for attn_key in ["attention", "coord"]])
                }
        ...
        # train discriminator
            ...
            loss, local_log_dict, local_predictions_dict = losses.discriminator_loss(
                model_output, batch, self, self.args
            )
            logging_dict.update(local_log_dict)
            predictions_dict.update(local_predictions_dict)
