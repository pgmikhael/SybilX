...
SERIES_DICOMKEYS = ['SeriesDescription', 'Manufacturer', 'ManufacturerModelName', 'WindowCenter', 'WindowWidth', 'SliceThickness', 'ImageType']
...
EXAM_METAKEYS = ['pn_patient_ID', 'pn_study', 'pn_study_desc', 'pn_studyuid', 'dicom_patient_ID', 'dicom_studyuid', 'studyuid_pn_dicom', 'seriesuid_pn_dicom', 'patient_ID', 'studyinstance_uid',
        'c_patient_ID_pn_dicom', 'c_studyuid_pn_dicom', 'c_patient_ID_pn_keys', 'c_studyuid_pn_keys', 'c_patient_ID_dicom_keys', 'c_studyuid_dicom_keys',
        'age_at_exam', 'sex', 'patient_location', 'pack_years', 'race', 'smoking_status', 'cancer_cohort_yes_no', 'lung_rads', 'lung_cancer_screening', 'diff_days_exam_lung_cancer_diagnosis',
        'exam_description', 'download', 'bridge_uid', 'diff_days', 'IV_contrast', 'comment_AT']
SERIES_METAKEYS = ['pn_series', 'pn_slice', 'pn_seriesuid', 'dicom_seriesuid', 'seriesinstance_uid', 'c_seriesuid_pn_dicom', 'study_series']
...
parser.add_argument('--source_json_path', type = str, default = '/Mounts/rbg-storage1/datasets/MGH_Lung_Fintelmann/dicom_metadata.json')
parser.add_argument('--output_json_path', type = str, default = '/Mounts/rbg-storage1/datasets/MGH_Lung_Fintelmann/mgh_lung_dataset_june21.json')
parser.add_argument('--metadata_csv_path', type = str, default = '/Mounts/rbg-storage1/datasets/MGH_Lung_Fintelmann/key_footbridge_20210528')
parser.add_argument('--png_path_replace_pattern', type = str, nargs=2, default= ['dicoms', 'pngs'])
parser.add_argument('--error_json_path', type = str, default = None)

def log_error(errors, category, **args):
    error_dict = args
    print("{} ({})".format(category, ", ".join([key + "=" + args[key] for key in args.keys()])))
...
if __name__ == '__main__':
    ...
    find_pattern, replace_with = args.png_path_replace_pattern[0], args.png_path_replace_pattern[1]
    ...
    for row_dict in tqdm(source_json, desc='Processing metadata from {}'.format(args.source_json_path)):
        ...
        path = row_dict['dicom_path'].replace(find_pattern, replace_with).replace('.dcm', '.png')
        
        assert os.path.exists(path), f"Could not find path {path}!"

        if not row_dict['dicom_metadata'].get('SeriesInstanceUID', False):
            log_error(errors, "Series Instance UID not present in DICOM metadata!", path=path)
            continue
        ...
        if study_instance_uid not in study_instance_uids:
            log_error(errors, "Study Instance UID not present in Patient metadata CSV!", studyuid=study_instance_uid, path=path)
            continue
            
        slice_location = float(row_dict['dicom_metadata'].get('SliceLocation', -1))
        image_posn = float( literal_eval(row_dict['dicom_metadata'].get('ImagePositionPatient', '[-1]'))[-1] )


        # match metadata by StudyInstanceUID
        exam_meta_rows = patient_metadata[ patient_metadata['studyinstance_uid'] == study_instance_uid ]
        series_meta_rows = exam_meta_rows[ exam_meta_rows['seriesinstance_uid'] == series_id ]
        has_series_meta_row = len(series_meta_rows) > 0
        if not has_series_meta_row:
            log_error(errors, "No rows with given studyuid and seriesuid found in metadata CSV!", studyuid=study_instance_uid, seriesuid=series_id, path=path)
            continue
        ...
        exam_dict = extract_from_dict(row_dict['dicom_metadata'], EXAM_DICOMKEYS, replace_missing_with=-1)
        exam_dict.update( extract_from_series(exam_meta_rows.iloc[0], EXAM_METAKEYS) )

        pid = exam_dict["bridge_uid"]

        ## series data
        series_dict = extract_from_dict(row_dict['dicom_metadata'], SERIES_DICOMKEYS, replace_missing_with=-1)
        if has_series_meta_row:
            series_dict.update( extract_from_series(series_meta_rows.iloc[0], SERIES_METAKEYS) )
        ...
        if pid in pid_list:
            ...
            existing_exams = [ exam['studyinstance_uid'] for exam in json_dataset[pt_idx]['accessions'] ]
            ...
            if study_instance_uid in existing_exams:
                ...
                if series_id not in list(json_dataset[pt_idx]['accessions'][exam_idx]['image_series'].keys()):
                ...
                elif path not in json_dataset[pt_idx]['accessions'][exam_idx]['image_series'][series_id]['paths']:
                    ...
                    json_dataset[pt_idx]['accessions'][exam_idx]['image_series'][series_id]['image_posn'].append(image_posn)
            else:
    ...
    for pt_idx, pt_dict in tqdm(enumerate(json_dataset), desc='Post-Processing'):
        ...
        for exam_idx, exam_dict in enumerate(pt_dict['accessions']):
            for series_id in json_dataset[pt_idx]['accessions'][exam_idx]['image_series'].keys():
                # calculate slice numbering
                slice_locations = json_dataset[pt_idx]['accessions'][exam_idx]['image_series'][series_id]['slice_location'] 
                # TODO: should maybe be sorted in reverse direction instead
                json_dataset[pt_idx]['accessions'][exam_idx]['image_series'][series_id]['slice_number'] = np.argsort(slice_locations).tolist()
