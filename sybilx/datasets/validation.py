...
class CSVDataset(data.Dataset):
    ...
    def parse_csv_dataset(self, file_path):
        ...
        for _row in _reader:
            ...
            if unique_id in dataset_dicts:
                ...
                dataset_dicts[unique_id]["slice_locations"].append(
                    row["fileslice_position_path"]
                )
            else:
    ...
    def create_dataset(self, split_group):
        ...
        for mrn_row in tqdm(self.dataset_dicts, position=0):
            ...
            censor_time = (
                mrn_row["years_to_cancer"]
                if label
                else mrn_row["years_to_last_negative_followup"]
            )
            paths = order_slices(mrn_row["paths"], mrn_row["slice_locations"])
            try:
                series_object = Serie(
                    paths, label, censor_time, self.args.img_file_type, mrn_row["split"]
                )
            except Exception:
                continue
            ...
            dataset.append({"serie": series_object, "exam": mrn_row["unique_id"]})
