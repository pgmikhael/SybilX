...
parser.add_argument(
    "--nlst_abnormalities_csv",
    type=str,
    default="/Mounts/rbg-storage1/datasets/NLST/package-nlst-564.2020-01-30/NLST_564/nlst_564.delivery.010220/nlst_564_ct_ab_20191001.csv",
)
parser.add_argument(
    "--nlst_metadata_csv",
    type=str,
...
parser.add_argument(
    "--nlst_imagedata_csv",
    type=str,
    default="/Mounts/rbg-storage1/datasets/NLST/package-nlst-564.2020-01-30/NLST_564/nlst_564.delivery.010220/nlst_564_ct_image_info_20191001.csv",
)
parser.add_argument(
    "--test_google_splits",
    type=str,
...
if __name__ == "__main__":
    ...
    def make_metadata_dict(
        ...
        series_id,
        use_timepoint=False,
        use_timepoint_and_studyinstance=False,
    ):
        if use_timepoint_and_studyinstance:
            df = dataframe.loc[
                (dataframe.pid == int(pid))
                & (dataframe.study_yr == timepoint)
                & (dataframe.seriesinstanceuids == series_id)
            ]
        elif use_timepoint:
            df = dataframe.loc[
                (dataframe.pid == int(pid)) & (dataframe.study_yr == timepoint)
            ]
        else:
    ...
    for path in tqdm(source_json):
        ...
        if (
            ...
            or ("SliceLocation" not in dcm_keys)
        ):
            continue
        ...
        img_posn = float(
            literal_eval(row_dict["dicom_metadata"]["ImagePositionPatient"])[-1]
        )
        timepoint = int(
            row_dict["dicom_metadata"]["ClinicalTrialTimePointID"][-1]
        )  # convert from 'T1' to 1
        ...
        pixel_spacing = list(
            map(float, eval(row_dict["dicom_metadata"]["PixelSpacing"]))
        )
        slice_thickness = float(row_dict["dicom_metadata"]["SliceThickness"])
        path = (
            row_dict["dicom_path"]
            .replace("/data/rsg/mammogram", "/Mounts/rbg-storage1/datasets")
            .replace("nlst-ct", "nlst-ct-png")
            + ".png"
        )
        ...
        if len(processed_dict["pids"]) < processed_len:
            if pid not in processed_dict["pids"]:
                ...
                if len(processed_dict["pids"]) % 1200 == 0:
                    ...
                    print(
                        "Skipped {} processed participants in {} hrs".format(
                            len(processed_dict["pids"]), elapsed
                        )
                    )
        ...
        if (len(processed_dict["pids"]) == len(pid_list)) and (pid in pid_list):
            ...
            existing_exams = [
                exam["exam"] for exam in json_dataset[pt_idx]["accessions"]
            ]
            if exam in existing_exams:
                ...
                if (
                    series_id
                    in list(
                        json_dataset[pt_idx]["accessions"][exam_idx][
                            "image_series"
                        ].keys()
                    )
                ) and (
                    path
                    in json_dataset[pt_idx]["accessions"][exam_idx]["image_series"][
                        series_id
                    ]["paths"]
                ):
                    processed_dict["exams"].append(exam)
        ...
        if len(pid_list) == len(set(processed_dict["pids"])) and (pid not in pid_list):
            print(
                "Already processed {} participants, with {} exams, and {} dicoms".format(
                    len(pid_list),
                    len(set(processed_dict["exams"])),
                    len(set(processed_dict["dcms"])),
                )
            )
        ...
        exam_dict = {
            ...
            "date": date,
            "abnormalities": make_metadata_dict(
                abnormalities_data, pid, timepoint, series_id, use_timepoint=True
            ),
        }
        ...
        if pid in pid_list:
            ...
            if exam in existing_exams:
                ...
                if series_id not in list(
                    ...
                    json_dataset[pt_idx]["accessions"][exam_idx]["image_series"][
                        series_id
                    ]["slice_location"].append(slicelocation)
                    json_dataset[pt_idx]["accessions"][exam_idx]["image_series"][
                        series_id
            ...
            if (len(pid_list) > 0) and (len(pid_list) % 500 == 0):
                elapsed = round((time.time() - now) / 3600, 3)
                print(
                    "Processed {} participants in {} hrs".format(len(pid_list), elapsed)
                )
            pid_list.append(pid)
