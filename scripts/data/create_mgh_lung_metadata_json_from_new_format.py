...
EXAM_METAKEYS = ['patient_ID', 'bridge_uid', 'patient_IDs used in cohort 1', 'LR Score', 'indication', 'Smoking Status', 'Year Since Last Smoked',
                 'Smoking Cessation Guidance Provided', 'Packs Years', 'gender', 'age']
SERIES_METAKEYS = []
...
parser.add_argument('--png_path_replace_pattern', type = str, nargs=2, default= ['MIT_Lung_Cancer_Screening', 'screening_pngs'])
...
if __name__ == '__main__':
    ...
    for row_dict in tqdm(source_json, desc='Processing metadata from {}'.format(args.source_json_path)):
        ...
        exam_meta_rows = patient_metadata[ patient_metadata['Study Instance UID'] == study_instance_uid ]
        if len(exam_meta_rows) == 0:
            log_error(errors, "No rows with given studyuid found in metadata CSV!", studyuid=study_instance_uid, path=path)
            continue
        elif len(exam_meta_rows) > 1:
            log_error(errors, "Multiple rows with given studyuid found in metadata CSV! Choosing first!", studyuid=study_instance_uid, path=path)
        exam_meta = exam_meta_rows.iloc[0]

        series_ids = exam_meta[['1st Downloaded Series Instance UID', '2nd Downloaded Series Description', '3rd Downloaded Series Instance UID', '4th Downloaded Series Instance UID']].tolist()

        has_series_meta_row = series_id in series_ids
        ...
        series_id_download_index = series_ids.index(series_id)
        ...
        if pid in pid_list:
            ...
            existing_exams = [ exam['StudyInstanceUID'] for exam in json_dataset[pt_idx]['accessions'] ]
